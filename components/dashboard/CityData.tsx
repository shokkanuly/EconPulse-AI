...
import { KAZAKHSTAN_CITIES, CityEconomics } from "@/lib/api/kazakhstanData";
...
export function CityData({ userProfile }: CityDataProps) {
  ...
  // Calculate monthly household basket
  const getBasketCost = (city: CityEconomics) => {
    return (
      city.apartmentRent +
      city.utilities +
  ...
  const basketA = useMemo(() => getBasketCost(cityA), [cityA]);
  const basketB = useMemo(() => getBasketCost(cityB), [cityB]);
  ...
  const relocationAnalysis = useMemo(() => {
    ...
    const disposableA = curSal - basketA;
    const disposableB = tarSal - basketB;
