...
import { translateProfession, translateProfessionDesc } from "@/lib/translations";
...
export function FutureSalary({ indicators, country, parentMode, userProfile, onUpdateProfile }: FutureSalaryProps) {
  ...
  const chartData = useMemo(() => {
    ...
    for (let year = 1; year <= period; year++) {
      ...
      data.push({
        name: `${year}y`,
        [decayKey]: Math.round(realPurchasingPower),
        [matchKey]: Math.round(nominalNeeded),
        [targetKey]: Math.round(careerGrowth),
      });
    }
    return data;
